/**
  Powered by : Muhammad Fauzan & Yovani Rezakia Rezki
  Copyright  : 2024
**/
...
#define LED_FAILED_PIN 12
#define I2C_ADDR 0x27
LiquidCrystal_I2C lcd(I2C_ADDR, 16, 2);

// IP for server backend
String IP_ADDRESS = "192.168.1.129";
...
void succes_enroll(WebsocketsClient &client) {
...
void loop() {
  ...
  while (client.available()) {
    ...
    if (millis() - interval > door_opened_millis) { // current time - face recognised time > 5 secs
      digitalWrite(LED_SUCCESS_PIN, HIGH); //open relay
    }
        ...
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              sprintf(recognised_message, "ENROLL FOR %s", f->id_name);
              succes_enroll(client);
...
void hitApi(String id) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    String serverName = "http://" + IP_ADDRESS + ":8085/api/v1/employee/enroll/" + id; 
    HTTPClient http;
    ...
    if (httpResponseCode > 0) {
      ...
      const char* data = doc["data"]; // Mengambil nilai dari key "data"
      if (data) {
        Serial.print("Data: ");
        Serial.println(data);
        printToLCD(data);
//        buzzerSuccess();
        ...
        Serial.println("Error: data key not found in JSON response");
      }
      if (httpResponseCode != 200) {
        const char* errorData = doc["data"]; // Mengambil data error dari key "data"
        if (errorData) {
          Serial.print("Error data: ");
          Serial.println(errorData);
          printToLCD(errorData);
          delay(1000);
          lcd.clear();
        }
      }
...
void printToLCD(const char* text) {
  ...
  if (strlen(text) <= 16) {
    ...
    line1[16] = '\0'; // Ensure null-terminated string
    strncpy(line2, text + 16, 16);
    line2[16] = '\0'; // Ensure null-terminated string
    lcd.setCursor(0, 0);
    lcd.print(line1);
    lcd.setCursor(0, 1);
    lcd.print(line2);
